...
# PORT번호가 변경 peer0.org1.example:8051 orderer.example.com:11050
# ca_org1:7054 ca_org2:8054 ca_org3:10054 ca_orderer:11054 
...
function createOrg1() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:password1@localhost:7054 --caname ca-org1 --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
  { set +x; } 2>/dev/null
...
function createOrg3() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:password3@localhost:10054 --caname ca-org3 --tls.certfiles ${PWD}/organizations/fabric-ca/org3/tls-cert.pem
  { set +x; } 2>/dev/null
...
function createOrg2() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:password2@localhost:8054 --caname ca-org2 --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
  { set +x; } 2>/dev/null
...
function createOrderer() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:password0@localhost:11054 --caname ca-orderer --tls.certfiles ${PWD}/organizations/fabric-ca/ordererOrg/tls-cert.pem
  { set +x; } 2>/dev/null
