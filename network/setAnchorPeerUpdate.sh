...
pushd config
set -x
peer channel fetch config config_block.pb -o localhost:11050 --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
...
peer channel update -f Org2MSPAnchor.tx -c "${CHANNEL_NAME}" -o localhost:11050  --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA"
popd
...
infoln "Modify the configuration to append the anchor peer"
Modify the configuration to append the anchor peer 
jq '.channel_group.groups.Application.groups.Org3MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.org3.example.com","port": 10051}]},"version": "0"}}' config.json > modified_config.json
{ set +x; } 2>/dev/null
...
peer channel update -f Org3MSPAnchor.tx -c "${CHANNEL_NAME}" -o localhost:11050  --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA"
popd